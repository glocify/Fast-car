...
class CarsFEController extends Controller {
	...
	public function getMakesJson($id){
		...
		if(!$make){ response()->json($models); }
		$models1 = \App\MakeModel::whereRaw('make_id = ? AND status = ?', [$id, true,])->orderBy("name", "ASC")->get();
		foreach($models1 as $key => $value){
			$count = $cars->filter(function($item) use($value){ return (strpos(strtolower($item->model), strtolower($value->name)) !== false || strpos(strtolower($item->model), strtolower($value->value)) !== false); });
