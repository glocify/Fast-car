 ...
 /*       $ebay3 =   [
	...
	public function FilterSites(Request $request){
		...
		if($request->has('sites') && !empty($request->get('sites'))){
			...
			$resultJson = array_merge($carsforsaleJson, $ebayJson, $hemmingJson);
			...
			if(Input::has('file') && !empty(Input::get('file'))){
				...
				if(file_exists(public_path() . "/scraper_single/" . $file)){
					...
					if(isset($json['title']) && isset($json['href']) && isset($json['reference_id']) && isset($json['year']) && isset($json['model']) && isset($json['specs'])){
						...
						$json['sourceSite'] = (strpos($json['href'], strtolower('hemmings'))) ? 'Hemmings' : ((strpos($json['href'], strtolower('ebay'))) ? 'Ebay' : ((strpos($json['href'], strtolower('carsforsale'))) ? 'Carsforsale' : ((strpos($json['href'], strtolower('autotrader'))) ? 'Autotrader' : '' )));
						// $sites['single'] = 1;
						// $cars = $this->crawlCars($sites);
						// $car = "";
				...
				if(file_exists(public_path() . "/scraper_single/" . $file)){
					...
					if(isset($car['title']) && isset($car['href']) && isset($car['reference_id']) && isset($car['year']) && isset($car['model']) && isset($car['specs'])){
						...
						if(!in_array(strtolower($brand), array_map("strtolower", $excludeBrands))){
							...
							if(strpos($original_url, 'ebay') !== false || strpos($original_url, 'carsforsale') !== false || strpos($original_url, 'hemming') !== false){
								$pricerules = PriceRule::orderBy('site', 'DESC')->get();
	...
	public function postBulkAddListings(Request $request, $data = null){
		...
		if(count($carLinks) > 0){
			...
			if(file_exists($ebayPath) || file_exists($carsforsalePath) || file_exists($hemmingsPath)){
				$carsforsaleJson = [];
				$ebayJson = [];
				$hemmingsJson = [];
				...
				$resultJson = array_merge($carsforsaleJson, $ebayJson, $hemmingsJson);
				$cars = \App\Car::all();
				foreach($resultJson as $key => $car){
					...
					if(!in_array(strtolower($brand), array_map("strtolower", $excludeBrands))){
						...
						if(in_array($reference_id, $carLinks)){
							...
							if($exists->count() == 0){
								$carInserted = Car::create([ 'referenceID' => $reference_id, 'title' => $title, 'brand' => $brand, 'model' => $model, 'body' => '', 'year' => $year, 'mileage' => $mileage, 'seller' => $seller,
									'description' => $description, 'price' => $pricerule['price'], 'original_price' => $price, 'price_rule_id' => $pricerule['price_rule_id'], 'original_url' => $original_url, 'transmission' => $transmission, 'engine' => '', 'specification' => $specification,
								]);
	...
	public function postAddprice(Request $request){
		$validator = Validator::make($request->all(), [ 'start_price' => "required|integer", 'end_price' => "required|integer",
			'price' => "required|integer", 'percentage' => "required", 'site' => "required|in:all,ebay,carsforsale,hemmings",
		...
		if($pricerules->count() > 0){
			...
			if($cars->count() > 0){
				foreach($cars as $car){
					if(strpos($car->original_url, 'ebay') !== false || strpos($car->original_url, 'carsforsale') !== false || strpos($car->original_url, 'hemmings') !== false){
